...
def unscented_transform(points, U, f=None, points_noise=None, sigma_noise=None):
    ...
    if f is not None:
        if points_noise is None:
            points_pred = [f(points[i], U) for i in range(n_points)]
        else:
            points_noise = points_noise.points
            points_pred = [f(points[i], U, points_noise[i]) for i in range(n_points)]
    else:
        points_pred = points
...
def unscented_filter_predict(transition_function, points_state, U,
    ...
    (points_pred, moments_pred) = (
        unscented_transform(
            points_state, U, transition_function, 
...
def unscented_filter_correct(observation_function, moments_pred,
    ...
    (obs_points_pred, obs_moments_pred) = (
        unscented_transform(
            points_pred, U, observation_function,
...
def augmented_unscented_filter(mu_0, sigma_0, f, g, Q, R, Z, U):
    ...
    for t in range(T):
        ...
        if t == 0:
            ...
            (points_pred, moments_pred) = (
                unscented_filter_predict(
                    transition_function, points_state, U[t],
        ...
        mu_filt[t], sigma_filt[t] = (
            unscented_filter_correct(
                ...
                Z[t], U[t], points_observation=points_observation
            )
        )
